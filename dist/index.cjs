...
function getFileName(fileName, name, content, ext) {
  const hash = (0, import_hash_sum.default)(content);
...
`;
  ...
  async insert(insert) {
    ...
    if (this._options.styles.lang !== "css" && (this._options.styles.include === true || this._options.styles.include.includes("mixin"))) {
      template = await import_node_fs.promises.readFile(
...
var SVGManager = class {
  ...
  async update(filePath, loop = false) {
    ...
    if (!loop) {
      this.hash = (0, import_hash_sum2.default)(this.spritemap);
  ...
  get spritemap() {
    const DOM = new import_xmldom.DOMImplementation().createDocument(null, "", null);
  ...
  return {
    ...
    transform(code) {
      ...
      return {
        code: code.replace(
          spritemapFilter,
          import_node_path3.posix.join(base, fileNameWithHash)
        ),
        map: null
      };
    },
    transformIndexHtml(html) {
      const base = config.base.startsWith(".") ? config.base.substring(1) : config.base;
      return html.replace(spritemapFilter, import_node_path3.posix.join(base, fileNameWithHash));
    }
  };
}
