...
`;
  ...
  async insert(insert) {
    if (!this._options.styles || this._options.styles.include === false)
      return "";
    let template = "";
    if (this._options.styles.lang !== "css" && (this._options.styles.include === true || this._options.styles.include.includes("mixin"))) {
      template = await promises.readFile(
        path2.join(__dirname, `/template.${this._options.styles.lang}`),
        "utf8"
      );
    }
...
${template}`;
  ...
  // SCSS generation
  _generate_scss() {
    if (!this._options.styles || this._options.styles.include === false || Array.isArray(this._options.styles.include) && !this._options.styles.include.includes("variables")) {
      return "";
    }
    ...
    if (this._options.styles.include === true || this._options.styles.include.includes("bg-frag")) {
      if (this._options.output && this._options.output.view) {
        insert += this.createSpriteMap((name) => {
          ...
          sprite = `${selector} {`;
          sprite += `
	background: url('/${this._options.route}#${this._options.prefix + name}-view') center no-repeat;`;
          sprite += "\n}";
          return sprite;
        });
      }
...
var SVGManager = class {
  ...
  get spritemap() {
    ...
    this._svgs.forEach((svg, name) => {
      ...
      const documentElement = document.documentElement;
      let attributes = documentElement ? cleanAttributes(
      ...
      if (this._options.output && this._options.output.view) {
        ...
        attributes = documentElement && documentElement.attributes ? cleanAttributes(
          Array.from(documentElement.attributes),
          "view"
        ) : [];
        attributes.forEach((attr) => {
  ...
  async createFileStyle() {
    ...
    const path4 = resolve(this._config.root, this._options.styles.filename);
    await fs.writeFile(path4, content, "utf8");
  }
  get svgs() {
    return this._svgs;
  }
...
function BuildPlugin(iconsPattern, options) {
  ...
  const pluginExternal = new RegExp(`/${options?.route || "__spritemap"}`);
  return {
    ...
    async buildStart() {
      ...
      if (typeof options.output === "object") {
        ...
        const filePath = path3.join(config.build.assetsDir, fileName);
    ...
    transform(code) {
      ...
      return {
        code: code.replace(
          spritemapFilter,
          path3.join(base, fileNameWithHash)
        ),
        map: null
      };
    },
    transformIndexHtml(html) {
      const base = config.base.startsWith(".") ? config.base.substring(1) : config.base;
      return html.replace(spritemapFilter, path3.join(base, fileNameWithHash));
    }
  };
}
  ...
  return {
    ...
    async load(id) {
      ...
      if (!id.match(filterVueComponent))
        return;
      const [path4, query] = id.split("?", 2);
      const { name, base: filename } = parse(path4);
